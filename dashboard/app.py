...
nav = st.sidebar.radio("Navigation", ["Home", "Visualisasi"])
...
# ================================
# Halaman Home: Input Data Manual
# ================================
if nav == "Home":
    st.title("🏠 Home - Input Data Manual")
    st.markdown("Masukkan data baru di bawah ini. Data yang diinput akan digabungkan dengan data CSV (jika sudah diupload) dan akan ditampilkan di tabel.")
    
    # Daftar opsi untuk setiap kolom (sesuai dengan opsi yang diberikan)
    option_dict = {
    ...
    st.markdown("## Form Input Data Manual")
    # Gunakan st.form agar input dijalankan saat disubmit
    with st.form(key="data_form"):
        # Untuk tiap kolom pada option_dict, tampilkan widget selectbox (jika daftar opsinya tidak kosong) atau text_input
        input_data = {}
        for col, options in option_dict.items():
            if options:  # Jika ada daftar opsi
                input_data[col] = st.selectbox(f"{col}", options)
            else:
                input_data[col] = st.text_input(f"{col}", value="")
        # Tambahkan kolom yang bersifat numerik jika diperlukan
        input_data["pasien"] = st.number_input("Jumlah Pasien", min_value=0, step=1, value=0)
        input_data["date_start"] = st.text_input("Tanggal (YYYY-MM-DD)", value=datetime.today().strftime("%Y-%m-%d"))
        submit_button = st.form_submit_button(label="Submit Data")

    if submit_button:
        try:
            input_data["date_start"] = pd.to_datetime(input_data["date_start"])
        except Exception as e:
            st.error("Format tanggal tidak valid. Gunakan format YYYY-MM-DD.")
        # Buat DataFrame dari data manual yang diinput
        df_manual = pd.DataFrame([input_data])
        st.success("Data manual berhasil ditambahkan!")
        st.dataframe(df_manual)
        # Simpan atau gabungkan data manual ke dataset yang sudah ada
        # (Untuk contoh, data manual ditampilkan terpisah)
    ...
    # Opsi visualisasi di sidebar (tetap seperti sebelumnya)
    visualisasi_list = [
    ...
    # Pastikan ada data untuk diolah. Jika ada input manual sebelumnya, data CSV belum digabungkan.
    # Untuk contoh ini, kita gunakan data dari file CSV saja.
    if st.session_state.get("uploaded_file") is None and st.session_state.get("data_manual") is None:
        st.warning("Belum ada data. Silakan upload file CSV atau masukkan data manual di halaman Home.")
    else:
        # Jika data CSV ada, gunakan; jika tidak, gunakan data manual
        if not st.session_state.get("uploaded_file", pd.DataFrame()).empty:
            df = pd.read_csv(uploaded_file, sep=';', encoding='utf-8')
        else:
            df = pd.DataFrame()
    
    # Jika data ada, lakukan preprocessing & analisis
    if not df.empty:
        # Preprocessing: imputasi, hapus duplikasi, konversi tanggal
        ...
        # Pilihan visualisasi di halaman Visualisasi (sama seperti sebelumnya)
        ...
        st.warning("Data tidak tersedia. Silakan upload file CSV di halaman Home.")
